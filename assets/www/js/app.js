...
window.onpopstate = function(event)
{
    ...
    if (hash === "#page_home") {
		...
		$('.loading').hide();
	} else if (hash === "#page_info_dengue") {
		$("#li_page_info_dengue").addClass("active");
		setTimeout('init()',500);
	} else if (hash === "#page_overview") {
		$("#li_page_info_dengue").addClass("active");
		setTimeout('init()',500);
	} else if (hash === "#page_symptoms") {
		$("#li_page_info_dengue").addClass("active");
		setTimeout('init()',500);
	} else if (hash === "#page_grading") {
		$("#li_page_info_dengue").addClass("active");
		setTimeout('init()',500);
	} else if (hash === "#page_suggestion") {
		$("#li_page_info_dengue").addClass("active");
		setTimeout('init()',500);
	} else if (hash === "#page_prevention") {
		$("#li_page_info_dengue").addClass("active");
		setTimeout('init()',500);
	} else if (hash === "#page_test_form") {
		$("#li_page_test_form").addClass("active");
		$('#page_test_form').find('form')[0].reset();
	} else if (hash === "#page_help") {
		$("#li_page_help").addClass("active");
	} else if (hash === "#page_about") {
		$("#li_page_about").addClass("active");
	} else {
		// default link active
		$("#li_page_home").addClass("active");
		$('.loading').hide();
